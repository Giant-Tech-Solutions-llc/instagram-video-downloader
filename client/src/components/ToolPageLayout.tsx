                  ...
                  >
                    <div className="md:w-1/2 bg-black/5 relative aspect-video md:aspect-auto min-h-[200px]">
                      {processMutation.data.thumbnail ? (
                        ...
                        <img
                          src={`/api/proxy-image?url=${encodeURIComponent(processMutation.data.url)}`}
                          alt="Preview"
                          className="w-full h-full object-cover absolute inset-0"
                          data-testid="img-download-preview"
                        />
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
              {tool.features.map((feature, i) => (
                <div key={i} className="text-center group p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl md:rounded-[2rem] bg-[#F8F9FA] border border-black/5">
                  ...
                  <h3 className="text-sm sm:text-base md:text-lg font-black mb-2 sm:mb-3 text-[#1A1A1A]">{feature.title}</h3>
                  <p className="text-muted-foreground leading-relaxed text-xs sm:text-sm font-medium">{feature.desc}</p>
                </div>
              ))}
            </div>
          </div>
